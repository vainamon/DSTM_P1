...
cl::opt<bool> clAddCallToTMShutdown("tanger-add-shutdown-call", cl::init(true),
...
/**
 * begin() is replaced with the following code:
 ...
 * TODO get rid of the stack save/restore hack.
 */
...
void TransformLoadStore::init(Interface* tif, const char* suffix)
{
    this->tif = tif;
    if (!suffix) suffix = "";
    const Type* type_i8ptr = PointerType::getUnqual(IntegerType::get(
            tif->getModule()->getContext(), 8));
    const Type* type_voidptr = tif->getTypeVoidPtr();
    const Type* type_void = Type::getVoidTy(tif->getModule()->getContext());
    const Type* type_sizet = tif->getTypeSizet();
    ...
    std::string fn;
    fn.assign("tanger_stm_loadregionpre");
    fn.append(suffix);
    func_loadregionpre = tif->getOrCreateFunction(fn.c_str(), type_voidptr,
            type_voidptr, type_i8ptr, type_sizet, (const Type*) 0);
    fn.assign("tanger_stm_loadregionpost");
    fn.append(suffix);
    func_loadregionpost = tif->getOrCreateFunction(fn.c_str(), type_void,
            type_voidptr, type_i8ptr, type_sizet, (const Type*) 0);
    fn.assign("tanger_stm_storeregionpre");
    fn.append(suffix);
    func_storeregionpre = tif->getOrCreateFunction(fn.c_str(), type_voidptr,
            type_voidptr, type_i8ptr, type_sizet, (const Type*) 0);
    fn.assign("tanger_stm_updateregionpre");
    fn.append(suffix);
    func_updateregionpre = tif->getOrCreateFunction(fn.c_str(), type_voidptr,
            type_voidptr, type_i8ptr, type_sizet, (const Type*) 0);
}
...
{
    const Type* type_voidptr = tif->getTypeVoidPtr();
    int bits[loadstore_funcs_count] = { 8, 16, 16, 32, 32, 64, 64 };
    static const char* bitstr[loadstore_funcs_count] = { "8", "16",
            "16aligned", "32", "32aligned", "64", "64aligned" };
    for (unsigned i = 0; i < loadstore_funcs_count; i++) {
        const IntegerType* intty = IntegerType::get(
                tif->getModule()->getContext(), bits[i] < 8 ? 8 : bits[i]);
        PointerType* PointerTy_2 = PointerType::getUnqual(intty);
        std::vector<const Type*> FuncTy_15_args;
        FuncTy_15_args.push_back(type_voidptr);
        if (additionalArgs) FuncTy_15_args.insert(FuncTy_15_args.end(),
                additionalArgs->begin(), additionalArgs->end());
        FuncTy_15_args.push_back(PointerTy_2);
        const FunctionType* ft =
                FunctionType::get(intty, FuncTy_15_args, false);
        std::string name = tif_tanger_stm_load;
        name += nameModifier;
        name += bitstr[i];
    ...
    for (unsigned i = 0; i < loadstore_funcs_count; i++) {
        ...
        FuncTy_17_args.push_back(type_voidptr);
        if (additionalArgs) FuncTy_17_args.insert(FuncTy_17_args.end(),
                additionalArgs->begin(), additionalArgs->end());
        FuncTy_17_args.push_back(PointerTy_2);
        FuncTy_17_args.push_back(intty);
        const FunctionType* ft = FunctionType::get(Type::getVoidTy(
                tif->getModule()->getContext()), FuncTy_17_args, false);
        std::string name = tif_tanger_stm_store;
        name += nameModifier;
        name += bitstr[i];
        ...
        ValueMap<const Value*, Value*>* origToClone)
{
    ...
    for (BasicBlock::iterator i = bb->begin(), ie = bb->end(),
        ...
        // calls and invokes
            ...
            if (!f) {
                // indirect call
                // TODO do we have to do anything special for invokes?
                if (isa<InvokeInst>(&*i))
