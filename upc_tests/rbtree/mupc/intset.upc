...
int set_add(shared intset_t *set, intptr_t val, int notx) {
int res;
//printf("set add\n");
//fflush(stdout);
  if (notx) {
	...
	while (STOP == 0) {
		if ((rand_r(&SEED) % 100) < update) {
			if (last < 0) {
				...
				if (set_add(shared_rbtree, val, 0)) {
					diff++;
					last = val;
				}
				nb_add++;
...
int thr_per_node = THREADS/DEFAULT_NB_NODES;
...
int exp_size = 0;
	...
	if (MYTHREAD == 0) {
		...
		exp_size = set_size(shared_rbtree);
