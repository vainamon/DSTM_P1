...
int transfer(shared account_t *src, shared account_t *dst, int amount)
{
	int i;
...
shared int read_all = DEFAULT_READ_ALL;
shared int read_threads = DEFAULT_READ_THREADS;
shared int write_all = DEFAULT_WRITE_ALL;
shared int write_threads = DEFAULT_WRITE_THREADS;
...
void test()
{
	...
	seed[0] = (unsigned short)rand_r(&global_seed);
	seed[1] = (unsigned short)rand_r(&global_seed);
	seed[2] = (unsigned short)rand_r(&global_seed);
...
int main(int argc, char **argv)
{
	...
	if (MYTHREAD == 0)
	{
		...
		for (int i = 0; i < DEFAULT_NB_ACCOUNTS; i++) {
