...
shared node_t * new_node(intptr_t val, shared node_t *next, int gasnet_node) {
  ...
  node->node_lock = upc_global_lock_alloc();
  node->val = val;
  node->next = next;
...
shared intset_t * set_new() {
  ...
  int set_add(shared intset_t *set, intptr_t val, int notx, int gasnet_node) {
	...
	prev->next = new_node(val, next, -1);
      ...
      prev = set->head;
      upc_lock(prev->node_lock);
      next = prev->next;
      upc_lock(next->node_lock);
      while (1) {
	v = next->val;
	if (v >= val)
	  break;
	upc_unlock(prev->node_lock);
	prev = next;
	next = prev->next;
	upc_lock(next->node_lock);
      }
	...
	prev->next = n;
      ...
      upc_unlock(next->node_lock);
      upc_unlock(prev->node_lock);
      
      if (!result){
	upc_lock_free(n->node_lock);
	upc_free(n);
      }
    }
	...
	if (MYTHREAD == 0) {
	  ...
	  printf("Duration     : %d\n", duration);
		...
		for (i = 0; i < nb_threads; i++) {
		  ...
		  updates += (thread_stat[i].nb_add + thread_stat[i].nb_remove);
